...
export class AdminViewComponent {
  ...
  getAllStudent(): void {
    this.http.get('http://localhost:8089/api/users').subscribe(
      (resultData: any) => {
...
//users data
  getUserAdmin(email: string): void {
    this.http.get<any>(`http://localhost:8089/api/users?email=${email}`).subscribe(
      (resultData: any) => {
        ///console.log('message getUserAdmin Method:',resultData);
        const matchingStudents = resultData.data;
        // if (matchingStudents.length > 0) {
        //   this.adminData = matchingStudents[0];
        // }
        ...
        if (sessionEmail && matchingStudents) {
          ...
          //console.log('check sessionEmail:',adminData);
          if (adminData) {
            this.adminData = adminData;
            //console.log('display adminData:',adminData);
          }
        }
      },
      (error) => {
...
if(item.email===this.email){
  ...
  //update password
  this.userService.updatePass(body,this.id).subscribe(
    response => {
